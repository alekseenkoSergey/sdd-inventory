    ...
    <div class="stock-out-form">
      ...
      <form [formGroup]="form" (ngSubmit)="onSubmit()">
        ...
        <div class="form-actions">
          ...
          >
            {{ (loading$ | async) ? 'Submitting...' : 'Record Stock Out' }}
          </button>
          <button
            type="button"
...
export class StockOutFormComponent implements OnInit, OnDestroy {
  @Input() itemId!: number;
  ...
  ngOnInit(): void {
    if (this.currentQuantity) {
      ...
      if (quantityControl) {
        quantityControl.addValidators(StockMovementValidators.quantityMax(this.currentQuantity));
  ...
  onSubmit(): void {
    ...
    this.stockMovementService.createMovement(this.itemId, request)
      ...
      .subscribe({
        next: (movement) => {
          this.notificationService.success('Stock out recorded successfully');
  ...
  getErrorMessage(fieldName: string): string {
    ...
    if (errors['maxValue']) {
      return `Quantity cannot exceed current stock of ${errors['maxValue'].max} units`;
    }
    if (errors['maxLength']) {
